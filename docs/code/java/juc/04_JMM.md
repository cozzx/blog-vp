...
JMM（Java Memory Model，Java内存模型）是Java平台的一部分，它定义了Java内存访问的规则和同步的语义。JMM的主要目的是解决在多线程环境下Java内存访问的一致性问题，确保Java程序在各种平台上的正确执行。

JMM 描述了一组约定或规范，通过这组规范定义了程序中（尤其是多线程）各个变量的读写访问方式并决定一个线程对共享变量的写入以及如何变成对另一个线程可见，关键技术点都是围绕多线程的**原子性、可见性和有序性**展开的。
